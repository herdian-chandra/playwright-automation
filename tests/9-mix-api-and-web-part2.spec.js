...
test.beforeAll("Before All Step", async ({ browser }) => {
  // initialize
  ...
  // storage state
  await page.waitForLoadState("networkidle"); // wait for loading finish
  await context.storageState({ path: "state.json" }); // store storege session into state.json file
  // create a new context that already implement and have session storage state
  webContext = await browser.newContext({ storageState: "state.json" });
});

test("E2E Order item in Rahul Shetty Academy Ecom", async () => {
  /**
   * initialize storage state into new browser context
   */
...
test("Test 2 for antoher test case", async () => {
  /**
   * initialize storage state into new browser context
   */
  const page = await webContext.newPage();
